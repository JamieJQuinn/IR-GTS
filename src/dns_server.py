...
class DNSServer:
    ...
    def start_as_thread(self):
        dns_logging.info("DNSProxy server started. ")
        dns_logging.info("Primary DNS server: {}".format(self.proxy_ip))
        while True:
            dns_query, client_address = self.proxy_socket.recvfrom(512)[:2]
            dns_logging.debug("Received DNS query from {}".format(client_address))
